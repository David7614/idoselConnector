...
class CountriesFeed extends XmlFeed
{
    ...
    public function generate($what = null): int
    {
        ...
        echo "COUNTRIES FEED | user={$this->_user->username} queue={$this->_queue->id} page={$this->_queue->page}" . PHP_EOL;
        ...
        echo "batch done — updated={$updated} no_data={$noData}" . PHP_EOL;
        if (count($customersList) < self::BATCH_SIZE) {
            echo "less than batch size — done" . PHP_EOL;
            return 10;
        }
