...
class SubscribersFeed extends XmlFeed
{
    ...
    private $apiMethod          = '/api/admin/v5/clients/newsletter/email/search';
    // private $_request;
    ...
    private function getSubscribersBeginDate(): string
    {
        ...
        if (
            IntegrationData::getData('INITIAL_SUBSCRIBERS_DONE', $this->_user->id) &&
            $lastDate = IntegrationData::getDataValue('LAST_SUBSCRIBER_INTEGRATION_DATE', $this->_user->id)
        ) {
            return date('Y-m-d H:i:s', strtotime($lastDate . ' -1 week'));
        }

        $dateFrom = IntegrationData::getDataValue('LAST_SUBSCRIBER_INTEGRATION_DATE', $this->_user->id);
        if ($dateFrom) {
    ...
    private function createSubscriberObjects()
    {
        ...
        $begin = $this->getSubscribersBeginDate();
        ...
        try {
            ...
            $this->request_parameters['params']['shops']['shop_id'] = $approvalsShopId; // to nic nie zmienia bo to i tak pobiera wszystkich jak leci,
            // $this->request_parameters['params']['shops']['approval'] = 'y'; // to by zmieniło, ale wtedy zgubię wycofane zgody
            // $this->request_parameters['params']['shops']['registered'] = 'y'; // to że zarejestrowani tylko
        ...
        // die("!");
            ...
            foreach ($response['clients'] as $customer) {
                IdoselSubscriptions::processSubscriptionItem($this->_queue->getCurrentUser(), $approvalsShopId, $customer, 'newsletter', $this->_queue->page);
                continue;
            }
